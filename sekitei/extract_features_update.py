...
def extract_features_from_url(url):
    ...
    categories_templates = [
        ...
        'wiki_subcat_{}_{}:{}',           # wiki: Image:Deve.jpg
        'wiki_underlines_{}:{}',
        'wiki_spaces_{}:{}',
        'wiki_comma_{}:1',
        'wiki_all_spaces_{}:{}',
    ...
    for i, seg in enumerate(segments):
        ...
        subcategories = seg_name.split(':')
        if len(subcategories) > 1:
            for j, sub_group in enumerate(subcategories):
                features.append(categories_templates[6].format(i, j, sub_group))
        ...
        if ',' in seg_name:
            features.append(categories_templates[9].format(i))

        if counts_underlines + counts_spaces:
            features.append(categories_templates[10].format(i, counts_underlines + counts_spaces))

        if re.search(r'\(.*?\)', seg_name):
            features.append(categories_templates[11].format(i))
